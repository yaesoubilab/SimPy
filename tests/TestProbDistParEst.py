...
# dictResults=Est.fit_exp(dat_exp, 'Data')        # fit
# print("Fitting Exponential:", dictResults)
#
# 2 fitting a beta distribution
dat_beta = np.random.beta(2, 3, 1000)            # generate data
dictResults=Est.get_beta_parameters(np.mean(dat_beta), np.std(dat_beta)) # fit
...
# dictResults=Est.fit_binomial(dat_bin, 'Data', n=100, fixed_location=0) # fit
# print("Fitting Binomial:", dictResults)
#
# 5 Empirical (for int data)
dat_em = np.random.poisson(30, 1000)
dictResults=Est.fit_empirical(dat_em, 'Data') # fit
...
# dictResults=Est.fit_geometric(dat_geom, 'Data', fixed_location=0)        # fit
...
# dictResults=Est.fit_norm(dat_norm, 'Data')    # fit
